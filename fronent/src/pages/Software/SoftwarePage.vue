...
const populateFormOptions = async () => {
  ...
  try {
    ...
    if (areaField) areaField.options = areasRes.data.map(a => ({ label: a.aula, value: a.id }));

    const activosField = tipoFields.licencia_terceros.find(f => f.name === 'activos_asignados');
    if (activosField) activosField.options = activosRes.data.map(a => ({ label: `${a.codigo} - ${a.denominacion}`, value: a.id }));
