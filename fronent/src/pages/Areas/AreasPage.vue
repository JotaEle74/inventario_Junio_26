...
const openDialog = async (area = null) => {
  ...
  try {
    ...
    const oficinasOptions = res.data.map(ofi => ({ label: ofi.codigo + ' - ' +ofi.denominacion, value: ofi.id }));
...
const columns = [
  ...
  { name: 'oficina_codigo', label: 'CÓDIGO OFICINA', field: row=>row.oficina?.codigo, align: 'left' },
