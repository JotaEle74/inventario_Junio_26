...
<q-btn round flat icon="notifications" class="q-mr-sm">
  <q-badge v-if="totalExportPendientes > 0" color="red" floating>
    {{ totalExportPendientes }}
  </q-badge>
  ...
  <q-menu style="min-width: 350px">
    <q-list bordered separator>
      ...
      <q-item v-if="!exportStore.exports || exportStore.exports.length === 0" class="text-grey text-center q-pa-md">
        <q-item-section>No hay exportaciones recientes</q-item-section>
      </q-item>

      <q-item v-for="e in exportStore.exports" :key="e.id" class="q-py-md">
        <q-item-section avatar>
          <q-icon v-if="e.estado === 'procesando'" name="hourglass_empty" color="warning" class="notif-spin" />
          <q-icon v-if="e.estado === 'completado'" name="file_download" color="positive" />
          <q-icon v-if="e.estado === 'descargado'" name="check_circle" color="grey" />
          <q-icon v-if="e.estado === 'fallido'" name="error" color="negative" />
        </q-item-section>

        <q-item-section>
          <q-item-label lines="2">{{ e.mensaje }}</q-item-label>
          <q-item-label caption v-if="e.timestamp">
            {{ e.timestamp.toLocaleTimeString() }}
          </q-item-label>
          
          <q-btn v-if="e.estado === 'completado'" 
            label="Descargar" 
            color="primary" 
            size="sm" 
            flat 
            icon="download"
            @click="ejecutarDescarga(e)"
          />
        </q-item-section>

        <q-item-section side>
          <q-btn flat round dense icon="delete" size="xs" @click="exportStore.eliminarExport(e.id)" />
        </q-item-section>
      </q-item>
    </q-list>
  </q-menu>
  ...
  ...
          <q-menu>
            <q-list style="min-width: 200px">
              ...
              <!-- <q-item v-if="auth.isInventariador" clickable v-close-popup @click="pdfreport" :loading="loadingDescargarReportepdf">
                <q-item-section avatar>
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above bordered :width="240"
      ...
      <q-scroll-area style="height: calc(100% - 180px); margin-top: 180px">
        <q-list padding>
          ...
          <q-item v-if="auth.isSoftware" clickable v-ripple exact to="/software/reporte">
            <q-item-section avatar>
          ...
          <q-item v-if="auth.isAdmin" clickable v-ripple to="/movimientos">
            <q-item-section avatar>
          ...
          <q-item v-if="auth.isAdmin" clickable v-ripple to="/areas">
            <q-item-section avatar>
              <q-icon name="map" />
            </q-item-section>
            <q-item-section>Ubicaciones</q-item-section>
          </q-item>
          <q-item v-if="auth.isAdmin" clickable v-ripple to="/oficinas">
            <q-item-section avatar>
              <q-icon name="business" />
            </q-item-section>
            <q-item-section>Centro de costos</q-item-section>
          </q-item>
          <q-item v-if="auth.isInventariador||auth.isSupervisor" clickable v-ripple to="/inventariador">
            <q-item-section avatar>
              <q-icon name="assignment" />
            </q-item-section>
            <q-item-section>Inventariar Bienes</q-item-section>
          </q-item>
          <q-item v-if="auth.isSupervisor" clickable v-ripple to="/acta-verificacion">
            <q-item-section avatar>
              <q-icon name="assignment" />
            </q-item-section>
          ...
          <q-item v-if="auth.isAdmin || auth.isSupervisor" clickable v-ripple to="/usuarios">
            <q-item-section avatar>
              <q-icon name="people" />
            </q-item-section>
            <q-item-section>Usuarios</q-item-section>
          </q-item>
          <q-item v-if="auth.isAdmin || auth.isSupervisor" clickable v-ripple to="/configuracion">
            <q-item-section avatar>
...
const totalExportPendientes = computed(() => {
  return exportStore?.pendientes || 0
})
...
//const loadingDescargarReportepdf=ref(false)
/* async function pdfreport() {
  loadingDescargarReportepdf.value = true;
  try {
    const userOficina = await authService.getCurrentUser();
    const response = await activoService.getReportePdf({ id: userOficina.id });
    ...
    loadingDescargarReportepdf.value = false;
  }
} */
...
const ejecutarDescarga = async (exportItem) => {
  try {
    ...
    if (exportItem.export_id) {
      await activoService.eliminarExport(exportItem.export_id)
    }
  } catch (error) {
    $q.notify({ type: 'negative', message: 'Error al descargar', position: 'top' })
  }
}
